...
High-performance async implementation with connection pooling and concurrent processing
"""
...
class AsyncSpringDocsScraper:
    """Async Spring documentation scraper with high performance"""
    ...
    async def scrape_all(self):
        ...
        self.logger.info("Starting async Spring documentation scrape...")
    ...
    async def process_doc_url(self, url: str, metadata: Dict) -> Optional[ScrapedContent]:
        ...
        if doc_type == DocType.REFERENCE:
            return await self.process_reference_doc(project_name, url, soup, metadata)
        elif doc_type == DocType.API:
            return await self.process_api_doc(project_name, url, soup, metadata)
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(description='Async Spring documentation scraper')
    ...
    scraper = AsyncSpringDocsScraper(
        clean=args.clean,
